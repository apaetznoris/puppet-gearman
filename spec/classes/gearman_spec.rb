  ...
  [{},
   {
    ...
    describe "when #{param_set == {} ? 'using default' : 'specifying'} class parameters" do
      ...
      context 'on OS family Debian' do
        ...
        it { is_expected.to contain_package('gearman-job-server').with_ensure(package_ensure) }

        it do
          is_expected.to contain_file('/etc/default/gearman-job-server').with(
            'owner'   => 'root',
            'group'   => 'root'
        )
        end
        ...
        it do
          is_expected.to contain_service('gearman-job-server').with(
      ...
      context 'on OS family RedHat' do
        ...
        it do
          is_expected.to contain_service('gearmand').with(
            'ensure'     => param_hash[:service_ensure],
            'enable'     => param_hash[:service_enable],
            'hasrestart' => param_hash[:service_hasrestart],
            'hasstatus'  => param_hash[:service_hasstatus]
      ...
      %w(Solaris Darwin FreeBSD).each do |osfamily|
